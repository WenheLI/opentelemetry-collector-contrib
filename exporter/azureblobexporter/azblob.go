...
func NewAzBlobFileWriter(ctx context.Context, URL string, credential IAZIdentityCredential) (source.ParquetFile, error) {
	file := &AzBlockBlob{
		ctx:        ctx,
...
func NewAzBlobFileReader(ctx context.Context, URL string, credential IAZIdentityCredential) (source.ParquetFile, error) {
	file := &AzBlockBlob{
		ctx:        ctx,
		credential: credential,
	}
...
func (s *AzBlockBlob) Open(URL string) (source.ParquetFile, error) {
	...
	blobURL, err := azblob.NewBlockBlobClient(URL, s.credential, nil)
	if err != nil {
		return nil, err
	}
	fileSize := int64(-1)
	props, err := blobURL.GetProperties(s.ctx, nil)
	if err != nil {
